...
<1> Use https://docs.micronaut.io/snapshot/api/io/micronaut/validation/Validated.html[Validated] annotation at the class level to signal that this class requires validation.
...
<3> By default, `Content-Type` of Micronaut's response is `application/json` : override this with `text/plain` since we are returning a String, not a JSON object.
<4> The `@Get` annotation is used to map the index method to an HTTP GET request on `/books/stock/{isbn}`.
